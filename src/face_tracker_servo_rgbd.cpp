...
int main(int argc, char** argv)
{
    try
    {
        ...
        // get people detector based on rgbd camera
        ...
        while(!detector_initialized)
        {
            pepper_vs.getImage(image);
            vpDisplay::display(image);
            vpDisplay::flush(image);
            
            detector_initialized = people_detector.initializeDetector();
        }

        // get task
        pepper_visp::FaceWithDepthTask face_depth_task(camera_parameters, "face_tracker_servo_rgbd.yaml");
        ...
        while(true)
        {
            try
            {
                ...
                if(face_tracker.detectFace(image) && people_detector.isPersonVisible())
                {
